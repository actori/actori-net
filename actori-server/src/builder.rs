...
use actori_rt::time::{delay_until, Instant};
use actori_rt::{spawn, System};
...
impl ServerBuilder {
    ...
    where
        F: ServiceFactory<actori_rt::net::UnixStream>,
        N: AsRef<str>,
    ...
    fn handle_cmd(&mut self, item: ServerCommand) {
        match item {
            ...
            ServerCommand::Signal(sig) => {
                // Signals support
                // Handle `SIGINT`, `SIGTERM`, `SIGQUIT` signals and stop actori system
                match sig {
                    Signal::Int => {
