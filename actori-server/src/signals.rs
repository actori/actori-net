...
pub(crate) struct Signals {
    ...
    #[cfg(unix)]
    streams: Vec<(Signal, actori_rt::signal::unix::Signal)>,
}

impl Signals {
    pub(crate) fn start(srv: Server) -> io::Result<()> {
        actori_rt::spawn(lazy(|_| {
            #[cfg(not(unix))]
            {
                actori_rt::spawn(Signals {
                    srv,
                    stream: Box::pin(actori_rt::signal::ctrl_c()),
                });
            }
            #[cfg(unix)]
            {
                use actori_rt::signal::unix;
                ...
                actori_rt::spawn(Signals { srv, streams })
            }
        }));
